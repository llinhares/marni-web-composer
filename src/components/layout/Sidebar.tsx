  ...
  const handleMidiUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const getInstrumentFromMidi = (midiTrack: any): InstrumentType => {
        ...
        if (midiTrack.instrument.percussion) return 'Kit de Bateria';
        if (program === 24 || program === 25) return 'Violão Acústico';
        if (program === 43) return 'Contrabaixo';
        if (program === 46) return 'Harpa';
        if (program >= 40 && program <= 55) return 'Violino';
        if (program === 71) return 'Clarinete';
        if (program === 60) return 'Trompa';
        if (program >= 73 && program <= 79) return 'Flauta Transversal';
        return 'Grand Piano'; 
      };
      ...
      const newTracks: Track[] = midi.tracks.map((midiTrack, index) => ({
        ...
        instrument: getInstrumentFromMidi(midiTrack),
        volume: 70,
        isMuted: false,
        isSolo: false,
