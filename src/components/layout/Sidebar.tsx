...
function applySustainToNotes(notes: MidiNoteLike[], cc64Events: any[] = []): MidiNoteLike[] {
  ...
  const handleMidiUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newTracks: Track[] = midi.tracks.map((midiTrack, index) => {
        ...
        // Extract CC64 events for this track
        const cc64List = (midiTrack.controlChanges?.[64] || (midiTrack.controlChanges as any)?.['sustain'] || []) as any[];

        // Apply CC64 sustain processing
        const sustainedNotes = applySustainToNotes(midiTrack.notes, cc64List);
